...
`shared/vocabulary/{nouns,verbs,adjectives,other}.csv` from `docs/COMPETENCY_MAP.md`. Copy the
...
`shared/vocabulary/{nouns,verbs,adjectives,other}.csv` as they exist after the previous level was
finished, so no word is introduced twice.
...
1. AGENTS.md (repo root) -- current project state and invariants, especially "no vocabulary before
   it's taught" and the vocabulary CSV schema (four files, split by part of speech)
...
5. docs/VOCABULARY_STANDARD.md -- the exact CSV schema (nouns.csv, verbs.csv, adjectives.csv,
   other.csv) and per-column rules for each file
...
7. shared/vocabulary/nouns.csv, verbs.csv, adjectives.csv, other.csv (if they exist yet) -- every
   word already introduced at an earlier level; {{LEVEL}}'s new words must not duplicate these
...
1. Nouns: the correct article (de/het) -- do not guess; check a dictionary. The correct plural
   form and any irregularities. The diminutive form if natural and commonly used.
2. Verbs: whether it is regular/irregular/separable/reflexive; the ik-form, the shared
   jij/u/hij/zij/het present form, and the wij/jullie/zij present form; the simple past (singular
   and plural); the auxiliary (hebben/zijn); and the past participle.
3. Adjectives: the comparative, superlative, and inflected (attributive, +e) forms, watching for
   spelling changes (open/closed syllable rules) and irregular comparisons (e.g. goed/beter/best).
4. All parts of speech: a practical English-approximation pronunciation (not IPA), per
   docs/STYLE_GUIDE.md, and that the word is genuinely high-frequency/useful for {{LEVEL}}, not
...
4. Confirm no word selected for {{LEVEL}} already exists in shared/vocabulary/nouns.csv,
   verbs.csv, adjectives.csv, or other.csv from an earlier level (no duplicates) and no word
   needed by an earlier level's competency is missing from an earlier level (a gap that should
   have been caught earlier -- flag it if found, do not silently backfill into an earlier level's
   Status).
...
2. Add every selected word as a new row in the correct file for its part of speech, following the
   exact schema in docs/VOCABULARY_STANDARD.md:
   ...
   - Everything else -> other.csv: Dutch,English,Part of Speech,Pronunciation,CEFR Level,Lesson Introduced,Frequency Rank,Topic,Example Sentence,Example Translation,Notes
   (create a file with its header row if it does not exist yet). Leave "Lesson Introduced" as the
   ...
   docs/GRAMMAR_MAP.md and these same vocabulary files.
...
4. Update CHANGELOG.md with an entry: date, "Selected {{LEVEL}} vocabulary in
   docs/VOCABULARY_ROADMAP.md and shared/vocabulary/{nouns,verbs,adjectives,other}.csv," author
   ("AI agent" is fine), and reason.
...
4. Confirmation no duplicate words were introduced against the existing vocabulary files.
